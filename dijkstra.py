...
def dijkstra(graph, source, destination):
    ...
    distances = inf * np.array(ones(n)); #la distance de source à tous les sommets est l'infini
    ...
    while(unvisited(destination)): #tant qu'on est pas arrivé à destination:
        ...
        unvisitedSuccessors = graph[currentNode, :] * unvisited; #on sélectionne une ligne=prédécesseur (le sommet en cours) et toutes les colonnes=successeur. On multiplie par les non visitées, ce qui annule celles qui ont déjà été visitées. (dans le cas 0*inf on a une forme indéterminée).
        ...
        for node in unvisitedSuccessors:
            if np.isfinite(node) and node!=0: #si il vaut 0, l'infini ou nan, ce n'est pas un successeur non visité
                distances[i] = min(distances[currentNode] + node, distances[i]);
