...
class individu:
	...
	qualité
	"""
	def evaluation (self,autres_individus,distances):
		valeur_individu = 0
...
class population:
	...
	def selection (self , N=-1):
		...
		for v in self.individus:
			v.evaluation (self.individus,self.distances)
		
		self.individus.sort(key = lambda a : a.valeur_individu)
...
def genetic(matAdj):
    [n,n] = matAdj.shape;
    p = population(n,5+5*n); #population de 5 + n*5 individus: heuristique
    #initialise les distances
    for i in range(0,n-1):
        p.distances[i] = list(matAdj[i,i+1:n]);
        print(p.distances[0]);
    # n générations (heuristiques)
    # à chaque fois fait 50 + 10n croisements (heuristique) et sélectionne les n premiers (heuristique)

    for i in range(n):
        p.croisement_nombre (10*n+50)
        p.selection(n)
    for v in p.individus:
                        print(v.genes,v.calcul_distance (p.distances));

    #finalement, on garde le meilleur (même si pour des données adaptées à cet algo, on a en général plusieurs solutions aussi bonnes)
    p.selection(1);
    return(p.individus[0].genes);
